...
apt install -y curl wget python3-pip sqlite3 jq git tmux screen
...
while true; do
    ...
    echo "5️⃣ Freelance (Développement, Cybersécurité)"
    echo "6️⃣ Création & vente de scripts"
    echo "7️⃣ Afficher le solde et les transactions"
    echo "8️⃣ Quitter"
    ...
    case $choix in
        ...
        5)
            echo "💻 Inscris-toi sur Upwork, Fiverr ou GitHub Sponsors pour vendre tes services !"
            echo "$(date '+%Y-%m-%d %H:%M:%S'), Freelance, 100€" >> "$LOG_FILE"
            sleep 2
            ;;

        6)
            echo "📜 Vends tes scripts et outils sur Ko-Fi, Gumroad ou GitHub Sponsors !"
            echo "$(date '+%Y-%m-%d %H:%M:%S'), Vente de scripts, 50€" >> "$LOG_FILE"
            sleep 2
            ;;
